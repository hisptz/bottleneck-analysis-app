...
And("Set name of the intervention", () => {
  cy.get("[data-test=interventionName-label]").click();
  cy.get("#name").click();
  cy.get("#name").click();
  ...
  cy.get("[data-test=scorecard-admin-next-button]").click({ multiple: true, force: true });
  cy.get("[data-test=dhis2-uicore-buttonstrip]").click({ multiple: true });
  // cy.get(".Mui-expanded > .MuiIconButton-label > svg").click({ multiple: true });
  // cy.get("[data-test=add-indicator-button]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-select-input]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-singleselectoption]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-transferoption]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-transfer-actions-addindividual]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-transferoption]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-transfer-actions-addindividual]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-transferoption]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-transfer-actions-addindividual]").click({ multiple: true });
  // cy.get("[data-test=dhis2-uicore-button]").click({ multiple: true });
