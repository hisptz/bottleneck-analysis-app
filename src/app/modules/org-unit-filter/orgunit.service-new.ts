...
export class OrgUnitService {
  ...
  getSanitizedOrgUnitModel(orgUnitModel: OrgUnitModel): Observable<OrgUnitModel> {
    ...
    return Observable.create(observer => {
       ...
       */
      this._getOrgUnitLevelsInformation()
        .subscribe((orgUnitLevels: any[]) => {
          ...
          this._getOrgUnitGroups()
            .subscribe((orgUnitGroups: any[]) => {
               ...
               */
              this._getSanitizedUserOrgUnits(orgUnitModel.type)
  ...
  private _getOrgUnitLevelsInformation(): Observable<any[]> {
    return Observable.create(observer => {
      if (this._orgUnitDataStore.orgUnitLevels.length > 0) {
        observer.next(this._orgUnitDataStore.orgunitLevels);
        observer.complete();
      } else {
        this.http.get('organisationUnitLevels.json?fields=id,name,level&order=level:asc&paging=false')
  ...
  private _getOrgUnitGroups(): Observable<any[]> {
    return Observable.create(observer => {
  ...
  private _getUserOrgUnits(orgUnitType: string): Observable<any> {
  ...
  private _getSanitizedUserOrgUnits(orgUnitType: string): Observable<any[]> {
    return Observable.create(observer => {
      if (this._orgUnitDataStore.userOrgUnits.length > 0) {
        observer.next(this._orgUnitDataStore.userOrgUnits);
        observer.complete();
      } else {
        this._getUserOrgUnits(orgUnitType)
  ...
  private _getOrgUnits(urlFields: string, orgUnits: any[]) {
    return Observable.create(observer => {
      if (this._orgUnitDataStore.nodes.length > 0) {
        observer.next(this._orgUnitDataStore.nodes);
        observer.complete();
      } else {
        this.http.get('organisationUnits.json?fields=' + urlFields + '&filter=id:in:[' + orgUnits.join(',') + ']&paging=false')
          .subscribe((nodes: any) => {
