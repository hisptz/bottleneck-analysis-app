...
export class LegendSetService {
  ...
  public prepareThematicLayerLegendClasses(visualizationLayerSettings, visualizationAnalytics) {
    ...
    const legendsFromLegendSet: any = visualizationLayerSettings.legendSet ?
      this.getColorScaleFromLegendSet(visualizationLayerSettings.legendSet) : null;

    const legendSettings: any = {
      ...visualizationLayerSettings,
      colorScale: visualizationLayerSettings.legendSet ? legendsFromLegendSet.colorScale
        : this.colorInterpolation.getColorScaleFromHigLow(visualizationLayerSettings)
    };
  ...
  getFacilityLayerLegendClasses(visualizationLayerSettings, isLegendView) {
    ...
    const features: any = visualizationLayerSettings.geoFeature;
    ...
    groupSet.organisationUnitGroups.forEach(group => {
      ...
      if (!isLegendView) {
        visualizationLayerSettings.geoFeature = features.map(feature => {

          const featureIndex = _.findIndex(group.organisationUnits, _.find(group.organisationUnits, ['id', feature.id]));
