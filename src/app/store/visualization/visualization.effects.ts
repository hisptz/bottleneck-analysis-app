...
export class VisualizationEffects {
    ...
    flatMap((action: any) => {
      ...
      const analyticsPromises = _.map(
        ...
        (visualizationLayer: any) => {
           ...
           */
          const normalDxItems = _.filter(dxFilterObject ? dxFilterObject.items : [],
  ...
  @Effect({dispatch: false})
  resizeAction$ = this.actions$.ofType<visualization.ResizeAction>(
  ...
  private getNormalAnalyticsPromise(visualizationType: string, visualizationSettings: any,
