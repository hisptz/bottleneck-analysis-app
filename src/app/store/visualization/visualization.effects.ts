...
export class VisualizationEffects {
  ...
  setInitialVisualizations$ = this.actions$.ofType<dashboard.SetCurrentAction>(dashboard.DashboardActions.SET_CURRENT).
  ...
  @Effect()
  laodFavorite$ = this.actions$.ofType<visualization.LoadFavoriteAction>(
    visualization.VisualizationActions.LOAD_FAVORITE).pipe(mergeMap((action: any) => {
    ...
    return favoritePromise.pipe(map((favorite: any) => new visualization.LoadAnalyticsAction(
      visualizationHelpers.updateVisualizationWithSettings(action.payload, favorite))
    ), catchError((error) => of(new visualization.VisualizationErrorOccurredAction(action.payload.id, error))));
  }));

  @Effect()
  loadAnalytics$ = this.actions$.ofType<visualization.LoadAnalyticsAction>(
    visualization.VisualizationActions.LOAD_ANALYTICS).pipe(
    flatMap((action: any) => {
  ...
  @Effect()
  visualizationWithMapSettings$ = this.actions$.ofType<visualization.UpdateVisualizationWithMapSettingsAction>(
    visualization.VisualizationActions.UPDATE_VISUALIZATION_WITH_MAP_SETTINGS
  ).flatMap(action => this._updateVisualizationWithMapSettings(action.payload)).map(
  ...
  localFilterChange$ = this.actions$.ofType<visualization.LocalFilterChangeAction>(
    visualization.VisualizationActions.LOCAL_FILTER_CHANGE).pipe(
    map((action: any) =>
  ...
  @Effect({dispatch: false})
  resizeAction$ = this.actions$.ofType<visualization.ResizeAction>(visualization.VisualizationActions.RESIZE).
  ...
  deleteSuccess$ = this.actions$.ofType<visualization.DeleteSuccessAction>(
    visualization.VisualizationActions.DELETE_SUCCESS).pipe(
