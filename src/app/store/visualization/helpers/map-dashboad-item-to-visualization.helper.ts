...
function getLoadedStatus(dashboardItem: any) {
  return dashboardItem.type === 'USERS' || dashboardItem.type === 'REPORTS' ||
    dashboardItem.type === 'RESOURCES' || dashboardItem.type === 'APP' || dashboardItem.type === 'MESSAGES';
}
...
function getFavoriteDetails(dashboardItem: any) {
  ...
  const favoriteId = _.isPlainObject(dashboardItem[_.camelCase(dashboardItem.type)]) ?
    dashboardItem[_.camelCase(dashboardItem.type)].id : undefined;


  return dashboardItem.type && dashboardItem.hasOwnProperty(_.camelCase(dashboardItem.type)) ? {
    id: favoriteId,
...
function getLayerDetailsForNonVisualizableObject(dashboardItem) {
  return dashboardItem.type === 'USERS' || dashboardItem.type === 'REPORTS' ||
    dashboardItem.type === 'RESOURCES' || dashboardItem.type === 'APP' ? [{settings: dashboardItem, analytics: {}}] : [];
}
