...
import {
  ...
  getCurrentDashboardGlobalSelections
} from '../selectors';
...
export class DashboardVisualizationEffects {
  ...
  loadDashboardVisualizationsSuccess$: Observable<any> = this.actions$.pipe(
    ...
    withLatestFrom(this.store.select(getCurrentDashboardGlobalSelections)),
    tap(
      ([action, globalSelections]: [
        ...
        const visualizationObjects: Visualization[] = _.map(
          ...
          dashboardItem =>
            getStandardizedVisualizationObject(dashboardItem, globalSelections)
        );
