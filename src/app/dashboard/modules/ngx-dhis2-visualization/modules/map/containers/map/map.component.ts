...
export class MapComponent implements OnInit {
  ...
  transformVisualizationObject(visualizationConfig, visualizationLayers, id) {
    ...
    if (visualizationConfig.currentType === 'MAP') {
      ...
      const layersMetadata = getSplitedVisualizationLayers(
      ...
      analyticsKeys.map(analyticsKey => {
        if (analyticsKey) {
          if (
            crudVisualizationObject.analytics[analyticsKey].rows.length != 0
          ) {
            crudVisualizationObject.analytics[analyticsKey].rows.map(
              rowData => {
                if (rowData[4] === 'k1ZoaGAFVL3') {
                  geoFeatureSanitizedAnalytics[analyticsKey.toString()] =
                    crudVisualizationObject.analytics[analyticsKey];
                  analytics = geoFeatureSanitizedAnalytics;
                  const newAnalytics = {
                    ...crudVisualizationObject,
                    analytics
                  };
          ...
          console.log(
            '[MAP Analytics] Missing analytics in the Visualization Object.'
          );
        }
      });
      analytics = geoFeatureSanitizedAnalytics;
      const newAnalytics = { ...crudVisualizationObject, analytics };
      const layers = newAnalytics.layers
        .filter(layer => {
          return layersUIDs.includes(layer.id);
        })
        .map((layer, pos) => {
          pos === 0 ? (layer.visible = true) : (layer.visible = false);
          return layer;
        });
      const sanitizedVisualizationObject = {
        ...crudVisualizationObject,
        layers
      };
