...
export class FilterService {
  ...
  getOrgunitLevelsInformation () {
    return this.http.get(this.constant.root_url + 'api/organisationUnitLevels.json?fields=id')
  ...
  getOrgunitDetails (orgunit) {
    return this.http.get(this.constant.root_url + 'api/organisationUnits/'+orgunit+'.json?fields=id,name,level,children[id,name,level]')
      .map((response: Response) => response.json())
      .catch( this.handleError );
  }
  ...
  getInitialOrgunitsForTree (uid:string = null) {
    if( uid == null ){
      return this.http.get(this.constant.root_url + 'api/organisationUnits.json?filter=level:eq:1&paging=false&fields=id,name,level,children[id,name,level]')
        .map((response: Response) => response.json())
        .catch( this.handleError );
    }else{
      return this.http.get(this.constant.root_url + 'api/organisationUnits/'+uid+'.json?fields=id,name,level')
        .map((response: Response) => response.json())
        .catch( this.handleError );
    }
  }
