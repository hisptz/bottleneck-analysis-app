...
export class InterpretationService {
  ...
  getInterpretation(itemType, itemId): Observable<any> {
     return this.http.get(this.constants.root_url + 'api/interpretations.json?fields=name,id,user[name],comments[:all,user[id,name]],*&filter=' + itemType + '.id:eq:' + itemId + '&paging=false')
       .map(res => res.json())
