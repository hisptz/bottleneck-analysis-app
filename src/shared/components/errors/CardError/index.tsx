...
export default function CardError({ error, resetErrorBoundary }: ErrorBoundaryComponentProps): React.ReactElement {
  const networkError: boolean = error?.details?.httpStatusCode === 404;
  return (
    <div className="loader-container">
      ...
      {networkError && resetErrorBoundary ? <Button onClick={resetErrorBoundary}>{i18n.t("Try again")}</Button> : null}
    </div>
  );
}
