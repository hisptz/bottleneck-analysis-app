...
export default function InterventionChips({
	showAll,
	interventions,
}: InterventionChipsProps) {
	...
	const onChipClick = useCallback(
		(selectedId: string) => () => {
			if (id !== selectedId) {
				navigate(`/interventions/${selectedId}`);
			}
		},
		[navigate, id],
	);
			...
			>
				{interventions?.map((intervention) => (
					<Chip
						...
						icon={
							intervention?.bookmarks?.includes(user?.id) ? (
								<IconStarFilled24 />
							) : null
						}
