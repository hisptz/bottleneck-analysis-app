...
export const DataItems = selectorFamily<
	{ functions: Array<string>; dataItems: Array<string> },
	string
>({
	...
	get:
		...
		({ get }): { functions: Array<string>; dataItems: Array<string> } => {
			const { dataSelection } =
				get<InterventionConfig | undefined>(InterventionState(id)) ??
				{};
			if (!dataSelection) {
...
export const SubLevelOrgUnit = selectorFamily({
	...
	get:
		...
		({ get }) => {
			...
			if (!orgUnit) {
				return ["USER_ORGUNIT", "USER_ORGUNIT_CHILDREN"];
			}
			if (!orgUnit || orgUnit.id === userOrgUnit?.id) {
				const { orgUnitSelection } =
					get<InterventionConfig | undefined>(
				...
				if (orgUnitSelection.subLevel) {
					if (userOrgUnit) {
						if (
							userOrgUnit?.level < orgUnitSelection.subLevel.level
						) {
							const levels = get(OrgUnitLevels);
							const orgUnitLevel = find(levels, [
								"id",
								orgUnitSelection.subLevel?.id,
							]);
							if (orgUnitLevel) {
								return [`LEVEL-${orgUnitLevel?.level}`];
							}
							return [
								`LEVEL-${orgUnitSelection?.subLevel?.level}`,
							];
						}
					}
				}
