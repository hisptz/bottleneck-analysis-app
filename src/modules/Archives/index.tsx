...
function ArchiveComponent(): React.ReactElement {
	...
	const onPageSizeChange = useRecoilCallback(
		({ set, reset }) =>
			(pageSize: number) => {
	...
	return (
		<div className="column archive-main-container h-100">
			<Steps
				...
				enabled={helpEnabled}
				steps={ARCHIVE_INTERVENTION_CONFIGURATION_HELP}
				onExit={onHelpExit}
				initialStep={0}
			/>
			<ArchivesListHeader />
			<Suspense fallback={<FullPageLoader />}>
				<div className="archiveList-table">
					{archives.state === "loading" ||
					pagination.state === "loading" ? (
						<FullPageLoader />
					) : null}
					{archives.state === "hasError" &&
						archives.contents.toString().includes("404") && (
							<div className="column w-100 h-100 center align-center gap">
								<span className="icon-72">
									<IconArchive24 color={colors.grey700} />
								</span>
								<h2
									style={{ color: colors.grey800, margin: 0 }}
								>
									{i18n.t(
										"There are currently no archived interventions",
									)}
								</h2>
					...
					{archives.state === "hasValue" &&
						isEmpty(archives.contents) && (
							<div className="column w-100 h-100 center align-center gap">
								<span className="icon-72">
									<IconArchive24 color={colors.grey700} />
								</span>
								...
								<div className="column gap p-16">
									...
									<div className="paginationCell">
										<Pagination
											...
											pageCount={
												pagination.contents.pageCount
											}
											pageSize={
												pagination.contents.pageSize
											}
