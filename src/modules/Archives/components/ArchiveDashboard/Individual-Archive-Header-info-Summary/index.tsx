...
export default function IndividualArchiveHeaderInfoSummary(): React.ReactElement {
	...
	const onLiveLinkClick = () => {
		navigate(`/interventions/${config?.id}`);
	};
	...
	return (
		<div className="w-100 archive-header-info-summary">
			<NoticeBox
				title={i18n.t(
					"You are currently viewing an archived intervention",
				)}
			>
				<div style={{ gap: 8 }} className="column ">
					<span>
						...
						{` ${i18n.t("for")} ${i18n.t("organisation unit")} `}
						<strong>{orgUnit?.displayName}</strong>{" "}
						{` ${i18n.t("and")} ${i18n.t("period")} `}{" "}
						<strong>{period.name}</strong>.
					</span>
					<span
						onClick={onLiveLinkClick}
						className="notice-box-link"
					>{`${i18n.t("Go to live")} ${config.name}`}</span>
				</div>
			</NoticeBox>
		</div>
	);
