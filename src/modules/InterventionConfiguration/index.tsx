...
export default function InterventionConfiguration(): React.ReactElement {
	...
	const interventionName = useRecoilValue(
		InterventionDirtySelector({ id, path: ["name"] }),
	);
	...
	const [activeStep, setActiveStep] = useRecoilState<ConfigStep | undefined>(
		ActiveStep(id),
	);
	...
	const isLastStep = useMemo(() => {
		...
		return (
			steps.length ===
			findIndex(steps, (step) => step.label === activeStep.label) + 1
		);
	}, [activeStep, steps]);

	const history = useNavigate();
	const { openDeleteConfirm, onDelete, onConfirmDelete, onDeleteCancel } =
		useDelete();
	...
	return (
		<div className="configuration-main-container">
			<div className="stepper-config-header">
				<h2
					style={{ margin: 4 }}
				>{`${interventionName != "" ? i18n.t("Edit intervention") : i18n.t("New intervention")}`}</h2>
			...
			<ButtonStrip middle>
				<Button
					loading={saving}
					dataTest={"save-exit-intervention-button"}
				...
				{!isLastStep && (
					<Button
						loading={saveAndContinueLoader}
						dataTest={"save-continue-intervention-button"}
