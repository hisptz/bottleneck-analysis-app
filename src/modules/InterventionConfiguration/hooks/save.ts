...
export default function useSaveIntervention(): {
	...
	const resetIntervention = useRecoilRefresher_UNSTABLE(
		InterventionState(interventionId),
	);
		...
		({ snapshot }) =>
			async () => {
				try {
					...
					const oldIntervention = cloneDeep(
						await snapshot.getPromise(
							InterventionDirtyState(interventionId),
						),
					...
					if (validate(newIntervention)) {
						...
						onExitReset();
						history(`/interventions/${newIntervention.id}`);
					} else {
						show({
							message: i18n.t(
								"Intervention name is required. Please provide on in the general page.",
							),
							type: { info: true },
						});
					}
				...
				await form.handleSubmit(async (data: any) => {
					...
					try {
						...
						if (
							...
							resetIntervention();
							resetSummary();
							const currentStep = await snapshot.getPromise(
								ActiveStep(interventionId),
							);
							reset(ActiveStep(interventionId));
							set(
								ActiveStep(newIntervention?.id),
								(prevState) => {
									const index =
										findIndex(CONFIG_STEPS, {
							...
							history.replace(
								`${newIntervention?.id}/configuration`,
							);
						} else {
							await updateIntervention(
								engine,
