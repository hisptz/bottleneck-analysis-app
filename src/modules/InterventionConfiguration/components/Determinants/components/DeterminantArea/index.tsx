...
export default function DeterminantArea(): React.ReactElement {
	...
	return (
		<div className="determinant-container">
			<div
				style={hasError && { border: `1px solid ${colors.red500}` }}
				className="determinant-main"
			>
				<div className="determinant-main-header">
					<h3>{i18n.t("Determinants")}</h3>
					<Button
						className={"determinant-clear-all-config"}
						dataTest={"clear-determinant-button"}
						disabled={allEmpty}
						onClick={onClearAll}
					>
						{i18n.t("Clear all")}
			...
			{hasError && (
				<p style={{ color: `${colors.red500}`, margin: "4px 0" }}>
					{errorMessage}
				</p>
			)}
		</div>
